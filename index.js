...
constfilmList= document.querySelector('#films');
...
.then(function(data){
    // Loop through each movie in data and add it to the filmList
    data.forEach(function(movie){
        const listItem = document.createElement('li');
        // Defining an object list item , assigning a document.create method to create a list when the data is looped from db.json
        listItem.classList.add('film','item');
        listItem.textContent= movie.title;
        filmList.appendChild(listItem);
    });
    const films = document.querySelector('.body');
    // Looping a new movie in the data and fetching a new card for it
    data.forEach(function(movie){
